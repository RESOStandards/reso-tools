 ...
 */
export const FriendlyError = ({ title, message }: {
  readonly title?: string;
  readonly message: string;
}) => {
  const navigate = useNavigate();
  const quip = QUIPS[Math.floor(Math.random() * QUIPS.length)];
  const errorInfo = formatError(message);
    ...
    <div className="h-full flex items-center justify-center pt-12">
      <div className="max-w-md text-center px-6">
        ...
        {errorInfo.serverMessage && (
          <p className="text-xs text-gray-500 dark:text-gray-400 mb-8 font-mono bg-gray-50 dark:bg-gray-800 rounded-lg px-3 py-2 whitespace-pre-line">
            {errorInfo.serverMessage}
          </p>
        )}
