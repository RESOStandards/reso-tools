...
> **New here?** If you've never touched OData before, the short version is: it's a REST flavor where the URL itself describes the query. `GET /Property?$filter=ListPrice gt 200000&$top=10` is a complete, valid query — no body needed. The SDK gives you a typed builder so you don't hand-write those URLs, plus helpers for the things URLs can't express (auth, paging, error handling, metadata parsing).

## Audience

Developers building applications that consume RESO-compliant OData servers — listing aggregators, agent tools, MLS dashboards, market analytics, data sync jobs, AI assistants. If you're calling a RESO Web API from TypeScript or JavaScript, this is your SDK.
...
Requires Node.js 22 or later. Works in any ESM environment — no platform-specific deps.
...
If you already have a token — from your IdP, your environment, a config file — pass it directly:
...
That's it. Every call from this client now sends `Authorization: Bearer your-bearer-token`.
...
You write zero token-management code. If your server prefers HTTP Basic auth on the token endpoint instead of form parameters, set `credentialTransport: 'header'`. If it expects credentials in the query string, set `credentialTransport: 'query'`. Defaults to `'body'` because that's what most servers expect.

### Configuring from environment variables

Hardcoding credentials is a bad idea. The SDK ships with a helper that reads them from `process.env` — same code path whether the user supplied a bearer token or client credentials:
...
| `RESO_AUTH_TOKEN` | Bearer token (used if client credentials env vars are not set) |
...
`extractEntityData` strips the `@odata.*` annotations off the response so you get a clean object with just the field values. If you need the annotations (etag, edit link, etc.) use `extractAnnotations` instead.
...
If you want the URL but not the call — for logging, deep links, debugging — use the chainable URI builder:
...
The builder supports every OData system query option: `$select`, `$filter`, `$orderby`, `$top`, `$skip`, `$count`, `$expand`, `$search`, `$compute`, `$format`. It also handles compound keys for resources whose primary key spans multiple fields:
...
Server-side paging in OData uses `@odata.nextLink`. The naive way is to read it yourself and build a loop. The SDK has a helper that does it for you:
...
`followAllPages` walks the `@odata.nextLink` chain until there isn't one, dedupes nothing (the server is responsible for that), and returns a flat array. **Use it carefully** — for resources with millions of records, you almost certainly want streaming or batching instead. A `100,000-row` listing dump is *not* the right use of this helper.
...
The `prefer: 'representation'` option tells the server to return the created entity in the response body. Drop it (or pass `prefer: 'minimal'`) if you only need the status and you don't care about the post-create state.
...
await updateEntity(
  ...
  'ABC123',
  { ListPrice: 275000 },
  { prefer: 'representation', ifMatch: '"etag-value"' }
);
```

The `ifMatch` option sends an `If-Match` header for optimistic concurrency control. Pass the etag you got from a prior read; the server returns 412 Precondition Failed if the record changed in between.
...
Parsed types you can work with: `CsdlEntityType`, `CsdlProperty`, `CsdlNavigationProperty`, `CsdlComplexType`, `CsdlEnumType`, `CsdlEntityContainer`, `CsdlEntitySet`, `CsdlSingleton`, `CsdlAction`, `CsdlFunction`. Use them to drive UI generators, build query validators, or sanity-check that a server actually exposes the resources your code assumes.
...
Useful for compliance work, but also for catching half-broken servers in production before they break your client. A schema that doesn't validate is a server you shouldn't trust to round-trip data.
...
This is what makes a server-aware client UI possible: parse the metadata once, validate the user's input against the entity type, surface field-level errors before any HTTP traffic.

---

## Handling errors

OData errors have a structured shape. The SDK gives you a type guard and a parser so you don't have to write defensive checks everywhere.
...
The SDK re-exports `parseFilter` from [`@reso-standards/odata-expression-parser`](../odata-expression-parser/) so you don't need a second dependency to work with `$filter` strings as ASTs.
...
This is what `validateQueryOptions` uses internally to check that filter property references are real. You can use it directly if you're building query UIs, translating between dialects, or feeding `$filter` expressions to a query planner.
...
| [query-with-filter.ts](https://github.com/RESOStandards/reso-tools/blob/main/reso-client/examples/query-with-filter.ts) | Query with `$filter`, `$select`, `$orderby`, `$top` |
