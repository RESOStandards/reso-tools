...
describe('validateQueryOptions', () => {
  ...
  it('accepts valid $select', () => {
    const result = validateQueryOptions({ $select: 'ListPrice,City' }, entityType);
    expect(result.valid).toBe(true);
  });

  it('rejects unknown $select field', () => {
    const result = validateQueryOptions({ $select: 'ListPrice,UnknownField' }, entityType);
  ...
  it('accepts valid $orderby', () => {
    const result = validateQueryOptions({ $orderby: 'ListPrice desc' }, entityType);
    expect(result.valid).toBe(true);
  });

  it('rejects unknown $orderby field', () => {
    const result = validateQueryOptions({ $orderby: 'Unknown asc' }, entityType);
    expect(result.valid).toBe(false);
    expect(result.errors[0].option).toBe('$orderby');
  });

  it('accepts valid $filter', () => {
    const result = validateQueryOptions({ $filter: "ListPrice gt 200000 and City eq 'Austin'" }, entityType);
    expect(result.valid).toBe(true);
  });

  it('rejects unknown $filter property', () => {
    const result = validateQueryOptions({ $filter: "UnknownField eq 'test'" }, entityType);
    expect(result.valid).toBe(false);
  ...
  it('rejects invalid $filter syntax', () => {
    const result = validateQueryOptions({ $filter: 'not valid odata at all !!!' }, entityType);
    expect(result.valid).toBe(false);
    expect(result.errors[0].option).toBe('$filter');
  });
  ...
  it('accepts valid $expand with known navigation property', () => {
    const result = validateQueryOptions({ $expand: 'Media' }, entityType);
    expect(result.valid).toBe(true);
  });

  it('accepts $expand with multiple navigation properties', () => {
    const result = validateQueryOptions({ $expand: 'Media,ListAgent' }, entityType);
    expect(result.valid).toBe(true);
  });

  it('accepts $expand with nested options', () => {
    const result = validateQueryOptions({ $expand: 'Media($select=MediaURL,MediaKey)' }, entityType);
    expect(result.valid).toBe(true);
  });

  it('rejects $expand with unknown navigation property', () => {
    const result = validateQueryOptions({ $expand: 'UnknownNav' }, entityType);
    expect(result.valid).toBe(false);
