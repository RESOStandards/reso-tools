...
The `preinstall` hook automatically builds sibling packages this one depends on (`odata-expression-parser`). To bootstrap every package at once, run `npm run bootstrap` from the repo root instead.
