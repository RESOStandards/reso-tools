...
the DD-lint venv that already has openpyxl + pytest:

    reso-certification/.venv-dd-lint/bin/python -m pytest reso-certification/utils/test_lint_dd_sheet.py

These tests are NOT part of `npm precommit` — the venv is intentionally kept out of git (see CLAUDE.md
DD-regeneration section). Run them when touching the linter or refreshing a DD sheet.
"""
